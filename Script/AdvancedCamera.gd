...
func zoom_to(selected_object: Employee) -> void:
	main_point = lerp(global_position, selected_object.global_position - Vector2(90,0), get_process_delta_time() * 12)
	zoom.x = move_toward(zoom.x, 1.35, get_process_delta_time() * 12)
	zoom.y = move_toward(zoom.y, 1.35, get_process_delta_time() * 12)
